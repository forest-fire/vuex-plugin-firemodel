import { FireModelPluginError } from "../errors/index";
/**
 ...
 */
export function set(obj, dotPath, value, createIfNonExistant = true) {
    if (!dotPath) {
        throw new FireModelPluginError(`Attempt to set value into a dotPath but the dotPath was empty!`, "not-allowed");
    }
    ...
    allButLast.forEach((p) => {
        if (!ref[p]) {
            if (createIfNonExistant) {
                ...
                throw new FireModelPluginError(`The dotPath -- ${dotPath} -- does not exist in the passed in object. You must either expressly state that you want the object structure created or this a real error that must be addressed otherwise. The part of the path which this failed on was "${p}".`);
            }
        }
        else if (typeof ref[p] !== "object") {
            throw new FireModelPluginError(`Failed to set the path of "${dotPath}" of the passed in base object because the base object had a scalar value along that path and setting this would have changed the object's data structure in way which is not allowed! The scalar value was found in the "${p}" component of the path.`);
        }
        ref = ref[p];
    });
    ref[key] = value;
