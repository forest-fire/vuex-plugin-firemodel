...
class RecordCrudFailure extends FireModelError {
    constructor(rec, crudAction, transactionId, e) {
        ...
        const message = `Attempt to "${crudAction}" "${capitalize$2(rec.modelName)}::${rec.id}" failed [ ${transactionId} ] ${e ? e.message : "for unknown reasons"}`;
...
class MissingInverseProperty extends FireModelError {
    constructor(rec, property) {
        ...
        this.from = capitalize$2(rec.modelName);
        this.to = capitalize$2(fkMeta.modelName);
...
class MissingReciprocalInverse extends FireModelError {
    constructor(rec, property) {
        super("", "firemodel/missing-reciprocal-inverse");
        const fkMeta = rec.getMetaForRelationship(property);
        const message = `The model "${capitalize$2(rec.modelName)}" is trying to leverage it's relationship with the model "${capitalize$2(fkMeta.modelName)}" through the property "${property}" but it appears these two models definitions are in conflict. ${capitalize$2(rec.modelName)} has been defined to look for an inverse property of "${capitalize$2(fkMeta.modelName)}.${rec.META.relationship(property).inverseProperty}" but it is not defined (or not defined as a relationship) on the ${capitalize$2(fkMeta.modelName)}! Look at your model definitions and make sure this is addressed.`;
        this.message = message;
    }
}
 ...
 */
function set$1(obj, dotPath, value, createIfNonExistant = true) {
    if (!dotPath) {
...
modelRollup) => (modelKlass, key) => {
    ...
    if (modelRollup) {
        ...
        set$1(modelRollup, modelAndProp, Object.assign(Object.assign({}, get$1(modelRollup, modelAndProp)), meta));
    }
};
...
const index$1 = propertyReflector({
    isIndex: true,
    isUniqueIndex: false,
}, indexesForModel);
...
function props$1(proto, extension) {
    if (typeof extension === 'function')
        extension = extension(getProto(proto));
    keys(extension).forEach(function (key) {
        setProp$1(proto, key, extension[key]);
    });
}
var defineProperty = Object.defineProperty;
function setProp$1(obj, prop, functionOrGetSet, options) {
...
function derive(Child) {
    return {
        from: function (Parent) {
            ...
            setProp$1(Child.prototype, "constructor", Child);
            return {
                extend: props$1.bind(null, Child.prototype)
            };
        }
    };
}
...
function mapError(domError, message) {
    ...
    if ("stack" in domError) {
        setProp$1(rv, "stack", { get: function () {
...
var thenProp = {
    ...
    set: function (value) {
        setProp$1(this, 'then', value && value.prototype === INTERNAL ?
            thenProp :
            {
                get: function () {
                    return value;
...
props$1(DexiePromise.prototype, {
    then: thenProp,
...
if (typeof Symbol !== 'undefined' && Symbol.toStringTag)
    setProp$1(DexiePromise.prototype, Symbol.toStringTag, 'Dexie.Promise');
...
if (NativePromise) {
    if (NativePromise.allSettled)
        setProp$1(DexiePromise, "allSettled", function () {
    ...
    if (NativePromise.any && typeof AggregateError !== 'undefined')
        setProp$1(DexiePromise, "any", function () {
            var possiblePromises = getArrayOf.apply(null, arguments).map(onPossibleParallellAsync);
            return new DexiePromise(function (resolve, reject) {
                if (possiblePromises.length === 0)
...
var Dexie$1 = Dexie;
props$1(Dexie$1, __assign(__assign({}, fullNameExceptions), {
    ...
    derive: derive, extend: extend$1, props: props$1, override: override,
    Events: Events,
...
var moreThanThreePeriods$1$1 = /\.{3,}/g;
// polyfill Array.isArray if necessary
if (!Array.isArray) {
    Array.isArray = function (arg) {
        return Object.prototype.toString.call(arg) === "[object Array]";
...
function pathJoin$1$1(...args) {
    ...
    try {
        ...
        const result = removeSingleDotExceptToStart$1(doubleDotOnlyToStart$1(args.reduce(reducer, "").replace(moreThanThreePeriods$1$1, "..")));
        return result;
    }
    catch (e) {
        if (e.name.includes("pathJoin")) {
  ...
  ```
    ...
    _getPaths(rec, deltas) {
        const added = (deltas.added || []).reduce((agg, curr) => {
            agg[pathJoin$1$1(this.dbPath, curr)] = rec.get(curr);
            return agg;
        }, {});
        const removed = (deltas.removed || []).reduce((agg, curr) => {
            agg[pathJoin$1$1(this.dbPath, curr)] = null;
            return agg;
        }, {});
...
class List extends FireModel {
    ...
    _injectDynamicDbOffsets(dbOffset) {
        ...
        if (dbOffset.includes(":")) {
            throw new FireModelError(`Attempt to get the dbPath of a List where the underlying model [ ${capitalize$2(this.modelName)} ] has dynamic path segments which were NOT supplied! The offsets provided were "${JSON.stringify(Object.keys(this._offsets || {}))}" but this leaves the following uncompleted dbOffset: ${dbOffset}`);
        }
        return dbOffset;
    }
}
...
function buildRelationshipPaths(rec, property, fkRef, options = {}) {
    try {
        ...
        const pathToRecordsFkReln = pathJoin$1$1(rec.dbPath, // this includes dynamic segments for originating model
        property, 
        ...
        results.push({ path: pathJoin$1$1(rec.dbPath, "lastUpdated"), value: now });
        ...
        if (inverseProperty) {
            ...
            const pathToInverseFkReln = fkInverseIsHasManyReln
                ? pathJoin$1$1(fkRecord.dbPath, inverseProperty, rec.compositeKeyRef)
                : pathJoin$1$1(fkRecord.dbPath, inverseProperty);
            // Inverse paths
            results.push({
            ...
            results.push({
                path: pathJoin$1$1(fkRecord.dbPath, "lastUpdated"),
                value: now,
            });
        }
...
function createCompositeKey(rec) {
    ...
    const dynamicPathComponents = rec.dynamicPathComponents.reduce((prev, key) => {
        if (!model[key]) {
            throw new FireModelError(`You can not create a composite key on a ${capitalize$2(rec.modelName)} without first setting the '${key}' property!`, "firemodel/not-ready");
        }
...
function setWithType(prop, value, model) {
    if (!model.META.property(prop)) {
        throw new FireModelError(`When building a "typed" composite key based on the model ${capitalize$2(model.constructor.name)}, the property "${prop}" was presented but this property doesn't exist on this model!`, "firemodel/property-does-not-exist");
    }
...
function extractFksFromPaths(rec, prop, paths) {
    ...
    return paths.reduce((acc, p) => {
        const fkProp = pathJoin$1$1(pathToModel, prop);
...
paths, options = {}) {
    ...
    try {
        ...
        const event = {
            ...
            fks,
            paths,
            from: capitalize$2(rec.modelName),
            to: capitalize$2(fkRecord.modelName),
...
async function writeAudit(record, action, changes, options = {}) {
    ...
    await Record.add(AuditLog, {
        modelName: capitalize$2(record.modelName),
        modelId: record.id,
        action,
        changes,
    }, { db });
...
class Record extends FireModel {
    ...
    static async add(model, payload, options = {}) {
        ...
        try {
            ...
            if (e.code === "permission-denied") {
                ...
                throw new FireModelError(`Permission error while trying to add the ${capitalize$2(rec.modelName)} to the database path ${rec.dbPath}`, "firemodel/permission-denied");
            }
    ...
    static createWith(model, payload, options = {}) {
        ...
        if (options.setDeepRelationships === true) {
            throw new FireModelError(`Trying to create a ${capitalize$2(rec.modelName)} with the "setDeepRelationships" property set. This is NOT allowed; consider the 'Record.add()' method instead.`, "not-allowed");
        }
        ...
        segments.forEach((segment, idx) => {
            if (segment.slice(0, 1) === ":") {
                ...
                if (segment !== pathParts[idx]) {
                    throw new FireModelError(`The attempt to build a composite key for the model ${capitalize$2(r.modelName)} failed because the static parts of the path did not match up. Specifically where the "dbOffset" states the segment "${segment}" the path passed in had "${pathParts[idx]}" instead.`);
                }
            }
    ...
    static modelName(model) {
        const r = Record.create(model);
        return capitalize$2(r.modelName);
    }
    get data() {
        return this._data;
    }
    ...
    getMetaForRelationship(property) {
        ...
        if (!((_a = this.META.property(property)) === null || _a === void 0 ? void 0 : _a.isRelationship)) {
            throw new FireModelError(`Attempt to get a Record instance for a relationship contained on the "${capitalize$2(this.modelName)}" model failed. The property "${property}" is either not a property or is not a relationship property on this model!`, "invalid-relationship-lookup");
        }
        if (this.META.property(property).relType === "hasMany") {
            throw new FireModelError(`Attempt to get a Record instance for a a relationship contained on the "${capitalize$2(this.modelName)}" model failed because the relationship property ["${property}"] has a cardinality greater than 1. Use "getRecordsForRelationships" instead.`);
        }
    ...
    async update(props) {
        ...
        if (!meta.property) {
            throw new FireModelError(`There is a problem with this record's META information [ model: ${capitalize$2(this.modelName)}, id: ${this.id} ]. The property() method -- used to dig into properties on any given model appears to be missing!`, "firemodel/meta-missing");
        }
        ...
        if (Object.keys(props).some((key) => {
            ...
            if (!rootProperties) {
                throw new FireModelError(`While this record [ model: ${capitalize$2(this.modelName)}, id: ${this.id} ] does return a "META.property" function, looking up the property "${root}" has resulted in an invalid response [${typeof rootProperties}]`);
            }
    ...
    async set(prop, value, silent = false) {
        ...
        if (!meta) {
            throw new FireModelError(`There was a problem getting the meta data for the model ${capitalize$2(this.modelName)} while attempting to set the "${prop}" property to: ${value}`);
        }
    ...
    refs, options = {}) {
        ...
        if (!meta.relationship(property)) {
            throw new FireModelError(`Attempt to associate the property "${property}" can not be done on model ${capitalize$2(this.modelName)} because the property is not defined!`, `firemodel/not-allowed`);
        }
        if (!meta.relationship(property).relType) {
            throw new FireModelError(`For some reason the property "${property}" on the model ${capitalize$2(this.modelName)} doesn't have cardinality assigned to the "relType" (aka, hasMany, hasOne).\n\nThe META for relationships on the model are: ${JSON.stringify(meta.relationships, null, 2)}`, `firemodel/unknown`);
        }
        ...
        if (relType === "hasMany") {
            ...
            if (Array.isArray(refs)) {
                if (refs.length === 1) {
                    ...
                    throw new FireModelError(`Attempt to use "associate()" with a "hasOne" relationship [ ${property}] on the model ${capitalize$2(this.modelName)}.`, "firemodel/invalid-cardinality");
                }
            }
    ...
    async _adding(options) {
        ...
        if (!this.db) {
            throw new FireModelError(`An attempt to add a ${capitalize$2(this.modelName)} record failed as the Database has not been connected yet. Try setting FireModel's defaultDb first.`, "firemodel/db-not-ready");
        }
        ...
        try {
            ...
            throw new FireModelProxyError(e, `An ${capitalize$2(this.modelName)} [${this.id}] model was being added but when attempting to add in the relationships which were inferred by the record payload it ran into problems. The relationship(s) which had properties defined -- and which had a bi-lateral FK relationship (e.g., both models will track the relationship versus just the ${capitalize$2(this.modelName)} [${this.id} model) --  were: ${relationshipsTouched.join(", ")}`);
        }
        return this;
    }
}
...
class DexieDb {
    ...
    table(model) {
        ...
        if (!this.modelIsManagedByDexie(model)) {
            throw new DexieError(`Attempt to get a Dexie.Table for "${capitalize$2(r.modelName)}" Firemodel model but this model is not being managed by Dexie! Models being managed are: ${this.modelNames.join(", ")}`, "dexie/table-does-not-exist");
        }
    ...
    async all(options = {
        ...
        const results = c.toArray().catch((e) => {
            if (e.code === "NotFoundError" || e.name === "NotFoundError") {
                console.info(`No records for model ${capitalize$2(this.meta.modelName)} found!`);
                return [];
            }
            else {
                throw new DexieError(`Problem with list(${capitalize$2(this.meta.modelName)}).all(${JSON.stringify(options)}): ${e.message}`, `dexie/${e.code || e.name || "list.all"}`);
            }
        });
        return results || [];
    }
...
class DexieRecord {
    ...
    async get(pk) {
        return this.table.get(pk).catch((e) => {
            throw new DexieError(`DexieRecord: problem getting record ${JSON.stringify(pk)} of model ${capitalize$2(this.meta.modelName)}: ${e.message}`, `dexie/${e.code || e.name || "get"}`);
        });
    }
    /**
    ...
    async add(record) {
        if (this.meta.hasDynamicPath) {
            if (!this.meta.dynamicPathComponents.every((i) => record[i])) {
                throw new DexieError(`The model ${capitalize$2(this.meta.modelName)} is based on a dynamic path [ ${this.meta.dynamicPathComponents.join(", ")} ] and every part of this path is therefore a required field but the record hash passed in did not define values for all these properties. The properties which WERE pass in included: ${Object.keys(record).join(", ")}`, "dexie/missing-property");
            }
        }
        if (!record.id) {
            record.id = key();
        ...
        const pk = await this.table
            .add(record)
            .catch((e) => {
            throw new DexieError(`DexieRecord: Problem adding record to ${capitalize$2(this.meta.modelName)}: ${e.message}`, `dexie/${e.code || e.name || "add"}`);
        });
        return this.get(pk);
    }
    /**
    ...
    async update(pk, updateHash) {
        ...
        const result = await this.table
            ...
            .catch((e) => {
            throw new DexieError(`DexieRecord: Problem updating ${capitalize$2(this.meta.modelName)}.${typeof pk === "string" ? pk : pk.id}: ${e.message}`, `dexie/${e.code || e.name || "update"}`);
        });
        if (result === 0) {
    ...
    async remove(id) {
        return this.table.delete(id).catch((e) => {
            throw new DexieError(`Problem removing record ${JSON.stringify(id)} from the ${capitalize$2(this.meta.modelName)}: ${e.message}`, `dexie/${e.code || e.name || "remove"}`);
        });
    }
}
...
/** Used to check objects for own properties. */
var hasOwnProperty$1$2 = objectProto$2$1.hasOwnProperty;

/** Used to detect if a method is native. */
var reIsNative$1 = RegExp('^' +
  funcToString$1$1.call(hasOwnProperty$1$2).replace(reRegExpChar$1, '\\$&')
  .replace(/hasOwnProperty|(function).*?(?=\\\()| for .+?(?=\\\])/g, '$1.*?') + '$'
);
...
var _a$2 = Object.prototype, hasOwnProperty$6 = _a$2.hasOwnProperty, propertyIsEnumerable$1 = _a$2.propertyIsEnumerable;
/**
 * @enum
 *
 * @const {Object} SUPPORTS
...
class AbcApi {
        ...
        if (this.config.useIndexedDb) {
            ...
            if (local.records.length > 0) {
                if (this.hasDynamicProperties) {
                    ...
                    store.commit(`${this.vuex.moduleName}/${DbSyncOperation.ABC_INDEXED_DB_SET_VUEX}`, localResults);
                }
            }
            else {
    ...
    async getDiscrete(request, options = {}) {
        ...
        if (options.strategy === AbcStrategy.getFirebase) {
            ...
            getFromFirebase(this, requestIds).then(async (server) => {
                ...
                store.commit(`${this.vuex.moduleName}/${DbSyncOperation.ABC_FIREBASE_MERGE_VUEX}`, serverResponse);
            });
        }
        const response = await AbcResult.create(this, {
            type: "discrete",
    ...
    async loadDiscrete(request, options = {}) {
        ...
        if (options.strategy === AbcStrategy.loadVuex) {
            // load data into vuex
            store.commit(`${this.vuex.moduleName}/${DbSyncOperation.ABC_FIREBASE_SET_VUEX}`, serverResponse);
        }
