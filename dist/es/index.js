...
class AbcApi {
        ...
        if (this.config.useIndexedDb) {
            ...
            if (local.records.length > 0) {
                store.commit(`${this.vuex.moduleName}/${this.hasDynamicProperties
                    ...
                    : DbSyncOperation.ABC_INDEXED_DB_SET_VUEX}`, localResults);
            }
            else {
            ...
            queryFirebase(this, firemodelQuery, local).then(async (server) => {
                ...
                if (this.config.useIndexedDb) {
                    ...
                    store.commit(`${this.vuex.moduleName}/${this.hasDynamicProperties
                        ...
                        : DbSyncOperation.ABC_FIREBASE_SET_INDEXED_DB}`, serverResults);
                }
                store.commit(`${this.vuex.moduleName}/${this.hasDynamicProperties
                    ? DbSyncOperation.ABC_FIREBASE_SET_DYNAMIC_PATH_VUEX
                    : DbSyncOperation.ABC_FIREBASE_SET_VUEX}`, serverResults);
                // watch records
    ...
    async loadQuery(request, options = {}) {
        ...
        if (this.config.useIndexedDb) {
            ...
            if (options.strategy === AbcStrategy.loadVuex) {
                if (this.hasDynamicProperties) {
                    ...
                    switch (queryDefn.queryType) {
                        ...
                        case QueryType.where:
                            store.commit(`${this.vuex.moduleName}/${DbSyncOperation.ABC_FIREBASE_MERGE_INDEXED_DB}`, serverResults);
                            break;
                        case QueryType.all:
                            store.commit(`${this.vuex.moduleName}/${DbSyncOperation.ABC_FIREBASE_SET_DYNAMIC_PATH_INDEXED_DB}`, serverResults);
                            break;
                    }
                }
                else {
                ...
                store.commit(`${this.vuex.moduleName}/${DbSyncOperation.ABC_INDEXED_DB_SET_VUEX}`, serverResults);
            }
        }
        const response = await AbcResult.create(this, {
            type: "query",
    ...
    async getDiscrete(request, options = {}) {
        ...
        if (local.records.length > 0) {
            ...
            store.commit(`${this.vuex.moduleName}/${DbSyncOperation.ABC_INDEXED_DB_MERGE_VUEX}`, localResults);
        }
        // query firebase if getFirebase strategy in place
        let server;
        if (options.strategy === AbcStrategy.getFirebase) {
            ...
            getFromFirebase(this, requestIds).then(async (server) => {
                ...
                store.commit(`${this.vuex.moduleName}/${DbSyncOperation.ABC_FIREBASE_MERGE_VUEX}`, serverResults);
            });
        }
        const response = await AbcResult.create(this, {
            type: "discrete",
